...
class Gomoku:
    ...
    def check_winner(self, player, last_move=None):
        if last_move is None:
            # Fallback to original method if no last move is provided
        ...
        # Optimized check based on last move
        row, col = last_move
        if self.board[row][col] != player:
            return False
        ...
        for row in range(self.size):
            for col in range(self.size):
                if self.board[row][col] != 0:
                    continue
                self.board[row][col] = player
                if self.check_winner(player, last_move=(row, col)):
...
class GomokuGame:
    ...
    def ai_move(self):
        ...
        if move:
            ...
            if self.game.check_winner(2, self.game.last_move):
                self.status = "AI Wins!"
                self.game.game_over = True
                self.game.winner = 2
            elif self.game.is_board_full():
